...
BaseApp::run(HINSTANCE hInst, int nCmdShow) {
    // Inicializamos ventana y después todos los recursos del motor
...
BaseApp::init() {
    ...
    // Crear swap chain para manejar back buffer y presentación
    ...
    // Crear render target view del back buffer
    ...
    // Crear textura para profundidad
    ...
    if (FAILED(hr)) {
        ERROR("BaseApp", "init",
            ("Failed to initialize DepthStencil texture. HRESULT: " + std::to_string(hr)).c_str());
        return hr;
    }

    // Crear vista del depth stencil
    ...
    //----------------------------------------------------------------------------------
    // Input layout básico para posición y coordenadas de textura
    //----------------------------------------------------------------------------------
    ...
    // Pasamos la geometría al mesh component
    ...
    // Topología básica para triángulos
    ...
    // Cargar textura base del cubo
    hr = m_textureCube.init(m_device, "Cracked2", ExtensionType::PNG);
    if (FAILED(hr)) {
        ERROR("BaseApp", "init",
            ("Failed to initialize cube texture. HRESULT: " + std::to_string(hr)).c_str());
        return hr;
    }
...
BaseApp::update(float deltaTime) {
    ...
    // Tiempo usado para animación del cubo
    static float t = 0.0f;
    ...
    // Rotación simple del cubo sobre su eje Y
...
void
BaseApp::render() {
    // Limpiamos pantalla y preparamos render target
